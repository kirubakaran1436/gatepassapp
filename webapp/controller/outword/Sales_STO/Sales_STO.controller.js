sap.ui.define([
    ...
    function (Controller, MessageBox, MessageToast, Fragment, Filter, FilterOperator, JSONModel, UI5Date) {
        ...
        return Controller.extend("gatepass.controller.outword.Sales_STO.Sales_STO", {
            onInit: function () {
            ...
            this.getOwnerComponent().getModel("YY1_SALES_STO_CDS").read("/YY1_SALES_STO/$count", { /* Decalure Globally in the Create table Serial Number */
        ...
        OnDisplayPage : function(oEvent){
            ...
            // On_Bin_Select:function(){
            //     let Dataa = this.getView().byId("Bin_Select_H").getValue();
            //     let Data01 = Dataa.trim();
            //     if(Data01 === ""){
            //         this.getView().byId("Bin_Select_H").setValue("Yes");
            //     }else if(Dataa !== "Yes" || Dataa !== "No"){
            //         this.getView().byId("Bin_Select_H").setValue("Yes");
            //     }
            // },
            ...
            OnReceivedQtyCal: function(DoItem, OrderQuantity) {
                ...
                return new Promise(function(resolve, reject) {
                    ...
                    var oFilter = new sap.ui.model.Filter("Delivery_No_Item", sap.ui.model.FilterOperator.EQ, DoItem);
                    var oFilter1 = new sap.ui.model.Filter("Delivery_No", sap.ui.model.FilterOperator.EQ, DelNo); 
            ...
            OnPendingQtyCal: function(DoItem, OrderQuantity) {
                ...
                var that = this; // Store reference to 'this'
                    ...
                    return new Promise(function(resolve, reject) {
                        ...
                        var oModel001 = that.getView().getModel("YY1_SALES_STO_CDS");
                        var oFilters001 = [oFilter, oFilter1, oFilter2];
    
                        var CalData = 0;
                
                        oModel001.read("/YY1_ITEM_SALES_STO", {
                            filters: oFilters001,
                            success: function(oData) {
                                var aItems = oData.results;
                                console.log(oData)
            ...
            for (var i = 0; i < Table_Length; i++) {
                ...
                if (oBindingContext) {
                    ...
                    itemData.push({
                        ...
                        Id:Id,
                        Delivery_No_Item:Delivery_Item,
            ...
            oEntry.Id = Id;
            oEntry.Delivery_No = Delivery_Item;
            ...
            var oModel = this.getOwnerComponent().getModel("YY1_SALES_STO_CDS");

            var that = this;

            oModel.create("/YY1_SALES_STO", oEntry, {
                success: function (oData, oResponse) {
                    ...
                    that.getOwnerComponent().getModel("YY1_SALES_STO_CDS").read("/YY1_SALES_STO/$count", { /* Decalure Globally in the Create table Serial Number */
                    success: $.proxy(function (oEvent, oResponse) {
                        let Count = Number(oResponse.body) + 1; // This should be a number, no need to use Number()
                        let CountLen = Count.toString(); // Convert to string to get its length
                        let AddData = "20000";
